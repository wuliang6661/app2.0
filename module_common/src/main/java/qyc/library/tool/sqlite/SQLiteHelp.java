package qyc.library.tool.sqlite;
...
public class SQLiteHelp {
	 ...
	 * 异常:
	 * 
	 * @param insertTableAndValue
	 *            key=表名,value=键值对 void
	 */
	 ...
	 *            查询字段名,如id=? and name=?
	 * @param selectionArgs
	 *            查询字段值,如new String[]{"1","Castiel"} void
	 */
	 ...
	 * @param usedid
	 *            当前已经使用掉的ID号
	 * @return int 下一个可用的ID号
	 */
	public static String GetSMSID(SQLiteDatabase db) {
		Cursor cs = db.query("SMSIDList", null, "isUsed=?", new String[]{"0"}, null, null, null);
		if (!cs.moveToNext()) {
			cs.close();
			db.execSQL("UPDATE SMSIDList SET isUsed=0 where ID<>1");
			return "001";
		} else {
			int id = cs.getInt(cs.getColumnIndex("ID"));
			cs.close();
			db.execSQL("update SMSIDList set isUsed=1 where ID=" + id);
