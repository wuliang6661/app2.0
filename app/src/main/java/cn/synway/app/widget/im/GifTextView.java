package cn.synway.app.widget.im;
...
import cn.synway.app.widget.imutil.EmotionUtils;
import cn.synway.app.widget.imutil.GifOpenHelper;
...
public class GifTextView extends AppCompatTextView {
    ...
    private CharSequence myText; // 存储textView应该显示的文本
     ...
     * @param inputStr
     */
    private boolean parseText(CharSequence inputStr) {
        myText = inputStr;
     ...
     * @param text
     */
    public void setSpanText(Handler handler, final CharSequence text, boolean isGif) {
...
//            setText(myText);
            highlightKeywords(new SpannableString(myText));
        }
    }
...
//            gifTextView.setText(sb);
    ...
    /**
     * 设置文字显示，身份证号  手机号  车牌号区别显示
     */
    private static final String REGEX_MOBILE_EXACT = "((13[0-9])|(14[5,7])|(15[0-3,5-9])|(17[0,3,5-8])|(18[0-9])|(147)|(199))\\d{8}";
    private static final String REGEX_ID_CARD18 = "[1-9]\\d{5}[1-9]\\d{3}((0\\d)|(1[0-2]))(([0|1|2]\\d)|3[0-1])\\d{3}([0-9Xx])";
    private static final String REGEX_LICENSE_PLATE_NUM = "[京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼使领A-Z][A-Z][A-Z0-9]{4}[A-Z0-9挂学警港澳]";

    private void highlightKeywords(SpannableString ss) {
        int color = ContextCompat.getColor(getContext(), R.color.colorPrimary);

        List<Integer> idCardEnds = new ArrayList<>();

        Pattern mPattern = Pattern.compile(REGEX_ID_CARD18);
        Matcher mMatcher = mPattern.matcher(ss);
        while (mMatcher.find()) {
            final CharSequence cs = ss.subSequence(mMatcher.start(), mMatcher.end());
            idCardEnds.add(mMatcher.end() - 1);
            ss.setSpan(new ForegroundColorSpan(color), mMatcher.start(), mMatcher.end(), Spanned.SPAN_INCLUSIVE_EXCLUSIVE);
            ss.setSpan(new ClickableSpan() {
                @Override
                public void onClick(View widget) {
                    if (listener != null) {
                        listener.getData(0, cs.toString());
                    }
                }
            ...
            }, mMatcher.start(), mMatcher.end(), Spanned.SPAN_INCLUSIVE_EXCLUSIVE);
        }

        mPattern = Pattern.compile(REGEX_MOBILE_EXACT);
        mMatcher = mPattern.matcher(ss);
        while (mMatcher.find()) {
            if (idCardEnds.contains(mMatcher.end())) {
                idCardEnds.remove((Integer) mMatcher.end());
                continue;
            }
            final CharSequence cs = ss.subSequence(mMatcher.start(), mMatcher.end());
            ss.setSpan(new ForegroundColorSpan(color), mMatcher.start(), mMatcher.end(), Spanned.SPAN_INCLUSIVE_EXCLUSIVE);
            ss.setSpan(new ClickableSpan() {
                ...
                public void onClick(View widget) {
                    if (listener != null) {
                        listener.getData(2, cs.toString());
                    }
                }
        ...
        mPattern = Pattern.compile(REGEX_LICENSE_PLATE_NUM);
        mMatcher = mPattern.matcher(ss);
        while (mMatcher.find()) {
            final CharSequence cs = ss.subSequence(mMatcher.start(), mMatcher.end());
            ss.setSpan(new ForegroundColorSpan(color), mMatcher.start(), mMatcher.end(), Spanned.SPAN_INCLUSIVE_EXCLUSIVE);
            ss.setSpan(new ClickableSpan() {
                ...
                public void onClick(View widget) {
                    if (listener != null) {
                        listener.getData(1, cs.toString());
                    }
                }
    ...
    public onTextSpanClick listener;

    public void setOnTextSpanListener(onTextSpanClick listener) {
        this.listener = listener;
    }


    public interface onTextSpanClick {

        /**
         * @param type 0 身份证号  1 车牌号  2 手机号
         * @param data 号码
         */
        void getData(int type, String data);
    }
