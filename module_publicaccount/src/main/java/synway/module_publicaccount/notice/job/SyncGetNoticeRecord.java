...
/**
 * Created by leo on 2018/10/24.
 */

public class SyncGetNoticeRecord {
    private boolean isStop = false;


    public void start(int startIndex,int count,int type){

        if (isStop) {
            return;
        }
        new Thread(new MRunnable(startIndex,count,type)).start();
    }
    ...
    private class MRunnable implements Runnable{
        ...
        private int count;
        private int type;
        private ArrayList<Obj_PublicMsgNotice> resultList;


        public MRunnable(int startIndex, int count,int type) {
        ...
        @Override public void run() {
            ...
            if (type == 0) {
                ...
                sql = "select * from " + Table_PublicAccount_Notice._TABLE_NAME + " order by " +
                    Table_PublicAccount_Notice.PUBLIC_NOTICE_COL_LOCALETIME + " desc  " +
    ...
    public interface OnGetRecordResult{
        void onFinish(ArrayList<Obj_PublicMsgNotice> list);
    }
