...
public class ViewHolder_PicText extends AdapterTypeRender{

    private ArrayList<LinearLayout> lines = new ArrayList<>();// 所有已经创建的行，这些行有严格的顺序要求
    private View convertView;
    private TextView tvTitle, locationType, pointnum, usernameView,tvSDFTime;// 消息标题
    ...
    public ViewHolder_PicText(String publicGuid,Context context, IOnPublicChatItemClick onPublicChatItemClick,Activity activity) {
    ...
    @Override void setData(int position, Obj_PublicMsgBase obj) {
        if (obj.MsgType == 4) {
            render((Obj_PublicMsgPicTxt) obj, false);
        } else if (obj.MsgType == 5) {     //轨迹消息
            renderTypeTrail((Obj_PublicMsgTrail) obj, false);
            ...
            if (((Obj_PulibcMsgDynamicLocation) obj).points.size() > 0) {
                if (((Obj_PulibcMsgDynamicLocation) obj).points.get(0).type == 1) {
                    renderTypeGis((Obj_PulibcMsgDynamicLocation) obj, false);
                } else {
    ...
    public void render(Obj_PublicMsgPicTxt dataObj, Boolean ifbright) {
        ...
        try {
            ...
            //标题 和 头像
            if (TextUtils.isEmpty(dataObj.title)) {
                tvTitle.setText("无");
            ...
            //时间
            tvSDFTime.setText(ParseTime.parseDate(dataObj.localTime));
     ...
     */
    private LinearLayout.LayoutParams getLineLayoutParagrams(Obj_ViewBase obj_viewBase) {
            ...
            if (StringUtil.isNotEmpty(viewUrlTxt.url)) {
                ...
                lineView.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View view) {
                        onPublicChatItemClick.onUrlTextItemClick(viewUrlTxt);
                    }
                });
            }
        }
