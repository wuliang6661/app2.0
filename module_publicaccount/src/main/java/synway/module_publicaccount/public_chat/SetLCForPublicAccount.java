...
public class SetLCForPublicAccount {
	...
	/** 获取公众号未读数 */
	public static final HashMap<String,Integer> getUnReadCountMap(Context context, SQLiteDatabase db) {
		HashMap<String,Integer> hashMap = new HashMap<>();
		String sql="select * from "+Table_LastContact._TABLE_NAME + " WHERE "+ Table_LastContact.TARGET_TYPE+"=2 ";
		Cursor cursor=db.rawQuery(sql,null);
		while(cursor.moveToNext()){
			hashMap.put(cursor.getString(cursor.getColumnIndex(Table_LastContact.TARGET_ID)),cursor.getInt(cursor.getColumnIndex(Table_LastContact.UN_READ_COUNT)));
		}
