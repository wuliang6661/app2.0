...
/** global Vue */
// mixins中的index.is 相当于多个vue共用此方法或者变量 =>类似于Constants
exports.default = {
  methods: {
    /**
     * 数据搜索方法根据传入的数据list和搜索条件val值对数据进行过滤
     * @param {Array} list 数据
     * @param {String} val 搜索值
     * @return {Array} - 过滤后的数据
     */
    searchData: function searchData(list, val) {
      if (!val || val === '') {
        return list;
      }
      var arr = [];
  ...
  computed: {
    baseUrl: function baseUrl() {
      var url = '';
      // 调用网路url,如果是Web 访问，使用config.js下的/api,非Web访问，则用'http://192.168.63.88:8050'
      // 外网调用地址：http://172.16.1.129:8808
      // 内网调用地址：http://192.168.39.39:8020
      // 测试调用地址：http://192.168.0.12:8808
      // 本机调用地址：http://172.16.1.182:8808
      url = weex.config.env.platform === 'Web' ? '/api' : 'http://192.168.39.39:8020';
      return url;
    }
  }
};
...
__vue_options__.__file = "D:\\weex_work\\YjWeex\\src\\components\\checkbox\\MoreCheckBox.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-529b311e"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
var _goToUrl = __webpack_require__(18);

var _goToUrl2 = _interopRequireDefault(_goToUrl);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var router = __webpack_require__(19); /* global Vue */
...
// 将http方法绑定到全局
var HttpServer = __webpack_require__(289);
// const goToUrl = require('@/utils/goToUrl');

// 添加实例属性:你可能会在很多组件里用到数据/实用工具，但是不想污染全局作用域。
// 这种情况下，你可以通过在原型上定义它们使其在每个 Vue 的实例中可用。
// 设置 Vue.prototype.$http 为 HttpServer 的别名
Vue.prototype.$http = HttpServer;

// Vue.mixin 全局注册一个混合，影响注册之后所有创建的每个 Vue 实例。
// 谨慎使用全局混合对象，因为会影响到每个单独创建的 Vue 实例（包括第三方模板）
Vue.mixin(_goToUrl2.default);
...
/**
 * 跟Android交互:跳转地址; 寻找Android本地图片
 */
exports.default = {
  methods: {
    androidJump: function androidJump(url) {
      var jumpUrls = null;
      var jumpUrl = '';
      if (url.indexOf('/') > -1) {
        jumpUrls = url.split('/');
        jumpUrl = jumpUrls[jumpUrls.length - 1];
      } else {
        jumpUrl = url;
      }
      ...
      return 'file://assets/' + jumpUrl;
    },
    androidImgSrc: function androidImgSrc(url) {
      var imageNames = null;
      var imageName = '';
      if (url.indexOf('/') > -1) {
        imageNames = url.split('/');
...
__vue_options__.__file = "D:\\weex_work\\YjWeex\\src\\index.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
...
var _ConditionList = __webpack_require__(24);

var _ConditionList2 = _interopRequireDefault(_ConditionList);

var _OrganList = __webpack_require__(272);

var _OrganList2 = _interopRequireDefault(_OrganList);
...
// weex自带：modal 模块提供了以下展示消息框的 API：toast、alert、confirm 和 prompt。
//
//
//
//
...
exports.default = {
  ...
  components: {
    ConditionList: _ConditionList2.default,
    OrganList: _OrganList2.default,
  ...
  created: function created() {
    ...
    // 先从Android这获得userId,监听 “Weex_UserId” 获得userId
    globalEvent.addEventListener("Weex_BaseUrl", function (e) {
      _this.userId = e.UserId; // 赋值
    });
  },
  ...
  methods: {
    ...
    submit: function submit() {
      var organList = this.organList,
          ...
          classifyCheckedList = this.classifyCheckedList,
          organCheckedList = this.organCheckedList,
          userId = this.userId,
          pushTime = this.pushTime,
          textRes = this.textRes;

      this.$refs.ConditionList.getCheckedList();
      this.classifyList = this.$refs.ConditionList.classifyList;
      this.classifyCheckedList = this.$refs.ConditionList.checkList;
      this.organList = this.$refs.OrganList.organList;
      this.organCheckedList = this.$refs.OrganList.organData;
      this.pushTime = this.$refs.OrganList.pushTime;
      var organIdsList = [];
      this.organCheckedList.forEach(function (item) {
        if (item.organId) {
          organIdsList.push(item.organId);
        }
      });
      var classifyCheckedListParam = [];
      var classifyCheckedListParamName = [];
      this.classifyCheckedList.forEach(function (item) {
        var obj1 = {};
        var objName1 = {};
        obj1.parent = item.cLevel;
        objName1.parent = item.cLevel;
        obj1.checked = item.checked;
        // 根据children,判断all的状态，只要children中有1个false,all就为false
        var checkAll = true;
        var list = [];
        var listName = [];
        item.children.forEach(function (testItem, testIndex) {
          var obj = {};
          obj.clevel = testItem.cLevel;
          obj.checked = testItem.checked;
          list.push(obj);
          listName.push(testItem.cLevel);
          if (!testItem.checked) {
            checkAll = false;
          }
        });
        obj1.all = checkAll;
        obj1.children = list;
        objName1.children = listName;
        classifyCheckedListParam.push(obj1); // 父选择list
        classifyCheckedListParamName.push(objName1);
      });
      var param = {
        // userId 0571_U1008
        userId: this.userId,
        // 坑点：JSON.stringify 里外层 只能用一次，不然会有 多余的"//"
        // 查询资料参数
        params: JSON.stringify({
          condition: {
            types: classifyCheckedListParamName,
            organIds: organIdsList,
            cityCodes: ['330100', '110000', '330100', '330700', '331100', '330800', '330600', '330200', '330900', '331000', '330400', '330500']
          },
          time: {
            hour: 24,
            day: 30,
        ...
        // classifyList: JSON.stringify(this.classifyList)
        // organList: JSON.stringify(this.o  rganList),
        // 个人配置参数
        classifyCheckedList: JSON.stringify({
          data: classifyCheckedListParam,
          message: '获取人员分类成功',
          status: 'success'
        }),
        organCheckedList: JSON.stringify({
          data: this.organCheckedList,
          message: '获取组织列表成功',
          pushTime: this.pushTime,
          status: 'success'
        })
      ...
      // 网络访问
      this.$http.post(this.baseUrl + "/getInCityData", param).then(function (res) {
        if (res[0].status === "success") {
          modal.alert({
            message: "提交成功",
            duration: 0.3
          }, function (value) {
            weexModule.finishActivity();
          });
        } else {
          modal.alert({
            message: "提交失败",
...
__vue_options__.__file = "D:\\weex_work\\YjWeex\\src\\components\\ConditionList.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
...
var _MoreCheckBox = __webpack_require__(8);

var _MoreCheckBox2 = _interopRequireDefault(_MoreCheckBox);

var _ConditionSelect = __webpack_require__(256);

var _ConditionSelect2 = _interopRequireDefault(_ConditionSelect);
...
// globalEvent 用于监听持久性事件，例如定位信息，陀螺仪等的变化
var globalEvent = weex.requireModule('globalEvent');
// storage 是一个在前端比较常用的模块，可以对本地数据进行存储、修改、删除，并且该数据是永久保存的，除非手动清除或者代码清除。
...
exports.default = {
  mixins: [_index2.default],
  components: { MoreCheckBox: _MoreCheckBox2.default, ConditionSelect: _ConditionSelect2.default },
  data: function data() {
    return {
      userId: '',
      checked: false,
  ...
  beforeCreate: function beforeCreate() {
    ...
    // 先从Android这获得userId,监听 Weex_BaseUrl 获得userId
    globalEvent.addEventListener('Weex_BaseUrl', function (e) {
      storage.setItem('Weex_BaseUrl', e.BaseUrl);
      // this.config.allCheckedIcon = 'http://192.168.91.117:9005/src/assets/check_box.png'; //`${e.BaseUrl}check_box.png`;
      _this.config.allCheckedIcon = e.BaseUrl + 'check_box.png';
      // this.config.unCheckedIcon = 'http://192.168.91.117:9005/src/assets/uncheck_box.png'; //`${e.BaseUrl}uncheck_box.png`;
      _this.config.unCheckedIcon = e.BaseUrl + 'uncheck_box.png';
      // this.config.partCheckedIcon = 'http://192.168.91.117:9005/src/assets/partcheck_box.png'; //`${e.BaseUrl}partcheck_box.png`;
      _this.config.partCheckedIcon = e.BaseUrl + 'partcheck_box.png';
      _this.userId = e.UserId; // 赋值
      _this.getData();
      _this.changeCheckStatus();
    });
  },
  created: function created() {
    var _this2 = this;

    storage.getItem('Weex_BaseUrl', function (e) {
      // this.iconSwitch = 'http://192.168.91.117:9005/src/assets/switch_open.png'; //`${e.data}switch_open.png`;
      _this2.iconSwitch = e.data + 'switch_open.png';
      // this.noSwitch = 'http://192.168.91.117:9005/src/assets/switch_close.png' //`${e.data}switch_close.png`;
      _this2.noSwitch = e.data + 'switch_close.png';
      // this.hasSwitch = 'http://192.168.91.117:9005/src/assets/switch_open.png' //`${e.data}switch_open.png`;
      _this2.hasSwitch = e.data + 'switch_open.png';
    });
    // this.getData(); 调试用
    // this.changeCheckStatus(); 调试用
  },
  mounted: function mounted() {
    var _this3 = this;

    // 挂载中，生命周期第四步 <=> onResume()
    globalEvent.addEventListener('refreshLBS', function () {
      _this3.getData();
      _this3.changeCheckStatus();
    });
  },
  ...
  methods: {
    ...
    getData: function getData() {
      ...
      this.$http.get(this.baseUrl + '/getPersonClassifyList?userId=' + this.userId) // 0571_U1008
      .then(function (res) {
        res.data.forEach(function (item, index) {
          var obj1 = {};
          obj1.cLevel = item.parent;
          if (item.children.length > 0) {
            obj1.className = 'file';
          } else {
            obj1.className = 'nofile';
          }
          obj1.id = Number('10' + index);
          obj1.checked = item.checked;
          // 根据children,判断all的状态，只要children中有1个false,all就为false
          var checkAll = true;
          var list = [];
          item.children.forEach(function (testItem, testIndex) {
            ...
            obj.cLevel = testItem.clevel;
            obj.checked = testItem.checked;
            obj.id = Number('100' + testIndex);
            obj.fLevel = item.parent;
            if (testItem.children && testItem.children.length > 0) {
              obj.className = 'file';
            } else {
              obj.className = 'nofile';
            }
            list.push(obj);
            if (!testItem.checked) {
              checkAll = false;
          ...
          obj1.all = checkAll;
          obj1.children = list;
          _this4.checkList.push(obj1); // 父选择list
        });
      }).catch(function () {});
    },
    ...
    /**
     * 全选checkBox 选择
     * @param {Object} e checkBoxItem选择
     */
    moreCheckBoxChecked: function moreCheckBoxChecked(e) {
      var all = this.all,
          checked = this.checked,
          checkList = this.checkList; // 当方法中需要data里面的变量，必须要引导进来,变量中不能有大写字符
      ...
      this.checkList.forEach(function (item, index) {
        ...
        item.all = e.checked;
        item.children.forEach(function (listItem, listIndex) {
          // 联动父选择list,统一修改
          listItem.checked = e.checked;
        });
      });
    },
     ...
     * 改变全选下的父选择状态
     */
    changeCheckStatus: function changeCheckStatus() {
      var checkList = this.checkList,
          all = this.all,
          checked = this.checked;
      // 总共3个情况：全选状态： all:T,checked:T; 全不选状态： all:F,checked:F; 部分选择： all:F,checked:T;
      // 为了方便判断，初始值都不是此3个情况。

      var changedAll = true;
      var changedChecked = false;
      this.checkList.forEach(function (item, index) {
        if (!item.all) {
          // 当有个all为false,所有都为false
          changedAll = false;
        }
        if (item.checked) {
          changedChecked = true;
    ...
    getCheckedList: function getCheckedList() {
      ...
      this.checkList.forEach(function (item, index) {
        item.children.forEach(function (listItem, listIndex) {
          if (listItem.checked) {
            var obj = {};
            obj.cLevel = listItem.cLevel;
            obj.fLevel = listItem.fLevel;
            data.push(obj);
          }
        });
      });
...
exports.default = {
  ...
  computed: {
    checkIcon: function checkIcon() {
      var icon = this.icon,
          innerAllChecked = this.innerAllChecked,
          config = this.config,
          innerChecked = this.innerChecked;

      var mergeIcon = [].concat(_toConsumableArray(icon));
      config.allCheckedIcon && (mergeIcon[0] = config.allCheckedIcon);
      config.unCheckedIcon && (mergeIcon[2] = config.unCheckedIcon);
      config.partCheckedIcon && (mergeIcon[1] = config.partCheckedIcon);

      if (innerAllChecked) {
        return mergeIcon[0];
      }
      return mergeIcon[innerChecked ? 1 : 2];
    },
    textColor: function textColor() {
      var innerChecked = this.innerChecked,
          config = this.config;

      var checkedColor = config.checkedColor ? config.checkedColor : "#1296db";
      return innerChecked ? checkedColor : "#3d3d3d";
    }
  },
  watch: {
    checked: function checked(newChecked) {
  ...
  methods: {
    wxcCellClicked: function wxcCellClicked() {
      ...
      this.innerAllChecked = !innerAllChecked;
      // 错误写法：不能这样写，这样它还是赋值之前的值
      // this.innerChecked = innerAllChecked;
      // 正确写法：
      this.innerChecked = this.innerAllChecked;
      this.$emit("moreCheckBoxChecked", {
        value: value,
        id: id,
        className: className,
        checked: this.innerChecked,
        all: this.innerAllChecked
      });
    }
  }
};
...
exports.default = {
  ...
  beforeCreate: function beforeCreate() {
    var url = '';
    if (weex.config.env.platform === 'Web') {
      url = 'http://172.16.1.129:7000/FTPText/weexResource/iconfont.ttf';
    } else {
      // local:///font/iconfont.ttf 本地，本地比网络快一点，将iconfont.ttf放在assets下font文件夹
      // http://192.168.63.88:9060/FTPText/weexResource/iconfont.ttf 内网
      // http://172.16.1.129:7000/FTPText/weexResource/iconfont.ttf 外网
      // http://192.168.0.10:9205/FTPText/weexResource/iconfont.ttf 测试网
      url = 'local:///font/iconfont.ttf';
    }
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('wxc-cell', {
    attrs: {
      ...
      "hasBottomBorder": _vm.hasBottomBorder,
      "accessible": true,
      "ariaLabel": (_vm.title + ",状态为" + (_vm.checked ? '已选中' : '未选中') + "," + (_vm.all ? '已全选' : '未全选'))
    },
    on: {
      "wxcCellClicked": _vm.wxcCellClicked
    }
...
__vue_options__.__file = "D:\\weex_work\\YjWeex\\src\\components\\other\\ConditionSelect.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-193ee9cb"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
var _SelectIcon = __webpack_require__(259);

var _SelectIcon2 = _interopRequireDefault(_SelectIcon);

var _CheckBoxList = __webpack_require__(263);
...
exports.default = {
  components: { SelectIcon: _SelectIcon2.default, MoreCheckBox: _MoreCheckBox2.default, CheckBoxList: _CheckBoxList2.default },
  props: {
    // 分类名称
    classTitle: {
      type: String,
  ...
  created: function created() {
    ...
    storage.getItem("Weex_BaseUrl", function (e) {
      // this.iconSelect = 'http://192.168.91.117:9005/src/assets/ic_more.png'; //`${e.data}ic_more.png`;
      _this.iconSelect = e.data + 'ic_more.png';
      // this.noSelect = 'http://192.168.91.117:9005/src/assets/ic_more.png'; //`${e.data}ic_more.png`;
      _this.noSelect = e.data + 'ic_more.png';
      // this.hasSelected = 'http://192.168.91.117:9005/src/assets/ic_less.png'; //`${e.data}ic_less.png`;
      _this.hasSelected = e.data + 'ic_less.png';

      // this.config.checkedIcon = 'http://192.168.91.117:9005/src/assets/check_box.png'; //`${e.data}check_box.png`;
      _this.config.checkedIcon = e.data + 'check_box.png';
      // this.config.unCheckedIcon = 'http://192.168.91.117:9005/src/assets/uncheck_box.png'; //`${e.data}uncheck_box.png`;
      _this.config.unCheckedIcon = e.data + 'uncheck_box.png';
      // this.config.checkedDisabledIcon = 'http://192.168.91.117:9005/src/assets/check_box.png'; //`${e.data}check_box.png`;
      _this.config.checkedDisabledIcon = e.data + 'check_box.png';
      // this.config.unCheckedDisabledIcon = 'http://192.168.91.117:9005/src/assets/uncheck_box.png'; //`${e.data}uncheck_box.png`;
      _this.config.unCheckedDisabledIcon = e.data + 'uncheck_box.png';

      // this.moreConfig.allCheckedIcon = 'http://192.168.91.117:9005/src/assets/check_box.png'; //`${e.data}check_box.png`;
      _this.moreConfig.allCheckedIcon = e.data + 'check_box.png';
      // this.moreConfig.unCheckedIcon = 'http://192.168.91.117:9005/src/assets/uncheck_box.png'; //`${e.data}uncheck_box.png`;
      _this.moreConfig.unCheckedIcon = e.data + 'uncheck_box.png';
      // this.moreConfig.partCheckedIcon = 'http://192.168.91.117:9005/src/assets/partcheck_box.png'; //`${e.data}partcheck_box.png`;
      _this.moreConfig.partCheckedIcon = e.data + 'partcheck_box.png';
    });
    this.changeCheckStatus();
  },
  ...
  methods: {
     ...
     * @param {Object} e checkBoxItem选择
     */
    checkBoxListChecked: function checkBoxListChecked(e) {
      var list = this.list; // 当方法中需要data里面的变量，必须要引导进来,变量中不能有大写字符
    ...
    clickSelect: function clickSelect(e) {
      var noSelect = this.noSelect,
          hasSelected = this.hasSelected;

      this.iconSelect = e.src !== noSelect ? noSelect : hasSelected;
      this.isSelected = e.src !== hasSelected;
    },
...
__vue_options__.__file = "D:\\weex_work\\YjWeex\\src\\components\\other\\SelectIcon.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-4858351e"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
exports.default = {
  ...
  computed: {
    textColor: function textColor() {
      ...
      var checkedColor = '#1296db';
      return checked ? checkedColor : '#3d3d3d';
    }
  },
  methods: {
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    class: ['div', _vm.hasTopBorder && 'top-border', _vm.hasBottomBorder && 'bottom-border'],
    on: {
      "click": _vm.onClicked
    }
  }, [_c('img', {
...
__vue_options__.__file = "D:\\weex_work\\YjWeex\\src\\components\\checkbox\\CheckBoxList.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
...
exports.default = {
  ...
  methods: {
    checkBoxItemChecked: function checkBoxItemChecked(e) {
      if (e.checked) {
        this.checkedList.push(e);
      } else {
        // 找到选中的值的位置
        var index = this.checkedList.indexOf(e.id);
        this.checkedList.splice(index, 1); // 去除被反选的
      }
      this.$emit('checkBoxListChecked', {
...
__vue_options__.__file = "D:\\weex_work\\YjWeex\\src\\components\\checkbox\\CheckBox.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-d0e69008"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('wxc-cell', {
    attrs: {
      ...
      "hasBottomBorder": _vm.hasBottomBorder,
      "accessible": true,
      "ariaLabel": (_vm.cLevel + ",状态为" + (_vm.checked ? '已选中' : '未选中') + "," + (_vm.disabled ? '不可更改' : '点击可切换'))
    },
    on: {
      "wxcCellClicked": _vm.wxcCellClicked
    }
    ...
    slot: "title"
  }, [_vm._v(_vm._s(_vm.cLevel))]), _c('image', {
    staticClass: ["checkbox"],
    attrs: {
      "src": _vm.checkIcon
    }
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', _vm._l((_vm.list), function(item, i) {
    return _c('checkbox', _vm._b({
      ...
      on: {
        "checkBoxItemChecked": _vm.checkBoxItemChecked
      }
    }, 'checkbox', item, false))
  }))
},staticRenderFns: []}
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    attrs: {
      ...
      "config": _vm.moreConfig,
      "id": _vm.id,
      "all": _vm.all,
      "hasBottomBorder": false
    },
    on: {
      "moreCheckBoxChecked": _vm.moreCheckBoxChecked
    }
  })], 1), (_vm.isSelected) ? _c('div', {
    ref: "clickSelect",
    staticClass: ["condition-list"]
  }, [(_vm.list.length === 0) ? _c('text', {
    staticClass: ["no-data"]
  }, [_vm._v("暂无数据")]) : _vm._e(), (_vm.list.length > 0) ? _c('check-box-list', {
    attrs: {
      "list": _vm.list,
      "config": _vm.config
    },
    on: {
      "checkBoxListChecked": _vm.checkBoxListChecked
    }
  }) : _vm._e()], 1) : _vm._e()])
},staticRenderFns: []}
module.exports.render._withStripped = true
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', [_c('more-check-box', {
    attrs: {
      "title": "全选",
      "checked": _vm.checked,
      "config": _vm.config,
    ...
    return _c('div', {
      ...
      on: {
        "selectChanged": _vm.selectChanged
      }
    })], 1)
  })], 2)
},staticRenderFns: []}
...
__vue_options__.__file = "D:\\weex_work\\YjWeex\\src\\components\\OrganList.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-1def78ef"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
var _GridCheckList = __webpack_require__(275);

var _GridCheckList2 = _interopRequireDefault(_GridCheckList);

var _SwitchIcon = __webpack_require__(283);
...
exports.default = {
  mixins: [_index2.default],
  components: { GridCheckList: _GridCheckList2.default, SwitchIcon: _SwitchIcon2.default },
  data: function data() {
    return {
      userId: "",
      pushTime: true,
      ...
      customStyles: {
        ...
        checkedIcon: "",
        uncheckedIcon: "",
        color: "#3d3d3d", // 未选中字体颜色
        checkedColor: "#1296db", // 选中字体颜色
  ...
  beforeCreate: function beforeCreate() {
    ...
    // 先从Android这获得userId,监听 “Weex_UserId” 获得userId
    globalEvent.addEventListener("Weex_BaseUrl", function (e) {
      _this.customStyles.checkedIcon = e.BaseUrl + "check_box.png";
      _this.customStyles.uncheckedIcon = e.BaseUrl + "uncheck_box.png";
      // this.iconSwitch = 'http://192.168.91.117:9005/src/assets/switch_open.png'; //`${e.BaseUrl}switch_open.png`;
      _this.iconSwitch = e.BaseUrl + "switch_open.png";
      // this.noSwitch = 'http://192.168.91.117:9005/src/assets/switch_close.png'; //`${e.BaseUrl}switch_close.png`;
      _this.noSwitch = e.BaseUrl + "switch_close.png";
      // this.hasSwitch = 'http://192.168.91.117:9005/src/assets/switch_open.png'; //`${e.BaseUrl}switch_open.png`;
      _this.hasSwitch = e.BaseUrl + "switch_open.png";
      _this.userId = e.UserId; // 赋值
      _this.getData();
    });
  },
  ...
  methods: {
    ...
    getData: function getData() {
      ...
      var organData = this.organData,
          userId = this.userId,
          pushTime = this.pushTime,
          iconSwitch = this.iconSwitch,
          noSwitch = this.noSwitch,
          hasSwitch = this.hasSwitch,
          organList = this.organList;
      // 网络获取数据

      this.$http.get(this.baseUrl + "/getOrganList?userId=" + this.userId) //0571_U1008
      .then(function (res) {
        _this3.organData = res.data;
        var organList = [];
        res.data.forEach(function (item, index) {
          if (item.checked) {
            var obj = {};
        ...
        _this3.organList = organList;
        _this3.pushTime = res.pushTime;
        if (_this3.pushTime) {
          _this3.iconSwitch = hasSwitch;
        } else {
          _this3.iconSwitch = noSwitch;
        }
      }).catch(function () {});
    },
     ...
     * 改变定时推送开关状态
     */
    changePush: function changePush(e) {
      var _this4 = this;
      ...
      var param = {
        onOrOff: !this.pushTime ? 'on' : 'off',
        userId: this.userId
      };
      // 网络访问
      this.$http.post(this.baseUrl + "/updatePushTime", param).then(function (res) {
        if (res === "success") {
          modal.alert({
            message: "修改定时推送开关状态:" + !_this4.pushTime,
            duration: 0.3
          }, function () {});
          _this4.pushTime = param.pushTime;
          if (_this4.pushTime) {
            _this4.iconSwitch = hasSwitch;
          } else {
            _this4.iconSwitch = noSwitch;
          }
        } else {
          modal.alert({
            message: "修改定时推送开关状态失败",
            duration: 0.3
          }, function () {});
        }
      }).catch(function () {
...
__vue_options__.__file = "D:\\weex_work\\YjWeex\\src\\components\\gridcheck\\GridCheckList.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-4f339d34"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
var _GridOption = __webpack_require__(278);
...
exports.default = {
  components: { Option: _GridOption2.default },
  props: {
    // 标识, 当界面展示多个grid, 防止v-for :key重复
    id: {
      type: String,
  ...
  methods: {
    ...
    initList: function initList() {
      ...
      var dList = this.list.map(function (item, i) {
        var checked = item.checked;
        // 同时单选模式下只认为第一个checked为true的为有效值

        checked = !!checked && (!single || checkedCount === 0);
        if (item.checked) checkedCount += 1;
        return _extends({}, item, {
          checked: checked
        });
...
__vue_options__.__file = "D:\\weex_work\\YjWeex\\src\\components\\gridcheck\\GridOption.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-3ec9acf6"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
exports.default = {
  ...
  methods: {
    onClick: function onClick() {
      this.$emit('select', this.GridCheckBox);
    }
  }
};
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    attrs: {
      "accessible": true,
      "ariaLabel": (_vm.organName + "," + _vm.organId + "," + (_vm.checked?'已选中':'未选中'))
    },
    on: {
      "click": _vm.onClick
    }
  }, [(_vm.checked && _vm.checkedIcon) ? _c('image', {
    staticClass: ["image-checked"],
    attrs: {
      "src": _vm.checkedIcon
    }
  }) : _vm._e(), (!_vm.checked && _vm.uncheckedIcon) ? _c('image', {
    staticClass: ["image-checked"],
    attrs: {
      "src": _vm.uncheckedIcon
    }
  }) : _vm._e(), (_vm.organName) ? _c('text', {
    staticClass: ["text-title"],
    style: _vm.cTitleStyle
  }, [_vm._v(_vm._s(_vm.organName))]) : _vm._e()])
},staticRenderFns: []}
module.exports.render._withStripped = true
...
__vue_options__.__file = "D:\\weex_work\\YjWeex\\src\\components\\other\\SwitchIcon.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-5026afa9"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    attrs: {
      ...
      "hasTopBorder": true,
      "hasBottomBorder": true,
      "checked": _vm.pushTime
    },
    on: {
      "selectClick": _vm.changePush
    }
  })], 1)
},staticRenderFns: []}
module.exports.render._withStripped = true
...
// 参数处理
var argFun = function argFun(ret, resolve, reject) {
  if (ret.status === 200 && ret.ok) {
    return resolve(ret.data);
  }
  if (ret.status === 400) {
    return message('请求参数错误400');
  }
  if (ret.status === 404) {
    return message('找不到文件404');
  }
  if (ret.status === 401) {
    return message('没有权限401');
  }
  if (ret.status === 403) {
    return message('没有可用资源403');
  }
  if (ret.status === 500) {
    return message('服务器内部错误500');
  }
  return reject(ret);
};
// 统一请求 promise
...
http.get = function () {
  ...
  return promiseMethod('GET', url);
};
http.post = function () {
  var url = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '';
  var params = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '';
  return promiseMethod('POST', url, params);
};
http.put = function () {
  var url = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '';
  var params = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '';
  return promiseMethod('PUT', url, params);
};
module.exports = http;
