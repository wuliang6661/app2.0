...
public class LineChartMarkView extends MarkerView {
    ...
    public LineChartMarkView(Context context,List<String> lineNameList,List<String> xValueList,List<List<Float>> yValuesList,String labelAttr) {
        super(context, R.layout.line_chart_marker_item);
    ...
    public LineChartMarkView(Context context,List<String> lineNameList,List<String> xValueList,List<List<Float>> yValuesList,String labelAttr,boolean isRound) {
        super(context, R.layout.line_chart_marker_item);
        tvXValue = findViewById(R.id.tv_Xvalue);
        tvYValue = findViewById(R.id.tv_Yvalue);
        this.xValueList = xValueList;
    ...
    public void refreshContent(Entry e, Highlight highlight) {
        ...
        try {
            ...
            for (int i = 0; i < lineNameList.size(); i++) {
                ...
                if (i == 19 || i == (lineNameList.size() - 1)) {
                    //当遍历到最后一个值的时候无需加\n 换行
                    tvYValueStr = tvYValueStr + lineNameList.get(i) + "  " +
                        yValue+ labelAttr;
                } else {
                    tvYValueStr = tvYValueStr + lineNameList.get(i) + "  " +
                        yValue+ labelAttr + " \n";
