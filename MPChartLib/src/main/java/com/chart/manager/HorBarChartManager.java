...
public class HorBarChartManager {
    ...
    public HorBarChartManager(HorizontalBarChart barChart, Context context) {
    ...
    private void initChart(HorizontalBarChart barChart) {
        /***图表设置***/
        //是否展示网格线
...
//        xAxis.setLabelRotationAngle(-60);
     ...
     * @param color      柱状图颜色
     */
    private void initBarDataSet(BarDataSet barDataSet, int color, boolean isRound) {
        ...
        if (isRound) {
            barDataSet.setValueFormatter(new MonthlyIntegerYValueFormatter());
        }
    ...
    private void setData(List<Float> barYValues, List<String> barXValues, String barName, String markViewlabelAttr, boolean isRound) {
        ...
        initBarDataSet(barDataSet, ColorUtils.DEFAULT_COLORS[0], isRound);
...
//        barChart.setVisibleXRange(0,8);//需要在设置数据源后生效
    }
    ...
    private void setData(List<List<Float>> barYValuesList, List<String> barXValues, List<String> barNameList,String markViewlabelAttr,boolean isRound) {
        ...
        for (int i = 0; i < barYValuesList.size(); i++) {
            ...
            initBarDataSet(barDataSet, ColorUtils.DEFAULT_COLORS[i], isRound);
