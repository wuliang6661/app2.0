...
public class IMSClientBootstrap {
    ...
    public static void main(String[] args) {
        ...
        bootstrap.init(userId, token, hosts, 0, null);
    }

    public synchronized void init(String userId, String token, String hosts, int appStatus, IMSConnectStatusCallback listener) {
        if (!isActive()) {
            ...
            imsClient.init(serverUrlList, new IMSEventListener(userId, token), listener);
        }
    }
    ...
    public void updateAppStatus(int appStatus) {
        ...
        if (imsClient.getAppStatus() == appStatus) {   //已经设置过，无需重复设置相同值
            return;
        }
        Log.i("imsClient", "设置长连接前后台状态改变！！！！");
